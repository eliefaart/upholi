...
pub async fn route_get_photos_minimal(user: User) -> Result<HttpResponse> {
	let photos = DbPhoto::get_all_for_user_minimal(user.id).await.map_err(ErrorInternalServerError)?;
...
pub async fn route_get_photo(
	...
	let photo_id = req.match_info().get("photo_id").ok_or_else(|| ErrorNotFound(HttpError::NotFound))?;
...
pub async fn route_upload_photo(user: User, payload: Multipart) -> Result<HttpResponse> {
	let form_data = get_form_data(payload).await.map_err(ErrorBadRequest)?;
	...
	let photo = serde_json::from_slice::<EncryptedPhotoUpsert>(&bytes_data).map_err(ErrorBadRequest)?;
	...
	db_photo.insert().await.map_err(ErrorInternalServerError)?;
...
async fn download_photo(
	...
	let photo_id = req
		.match_info()
		.get("photo_id")
		.ok_or_else(|| ErrorBadRequest("Photo ID invalid or missing"))?;
	let photo = DbPhoto::get(photo_id)
		.await
		.map_err(ErrorInternalServerError)?
		.ok_or_else(|| ErrorNotFound(HttpError::NotFound))?;
...
pub async fn delete_photos(user_id: String, ids: &[&str]) -> Result<HttpResponse> {
	...
	for id in ids {
		let photo = DbPhoto::get(id).await.map_err(ErrorInternalServerError)?;
	...
	for photo in photos {
		delete_photo_files(&photo).await.map_err(ErrorInternalServerError)?;
	}
