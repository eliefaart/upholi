...
const AlbumsPage: FC<PageProps> = (props: PageProps) => {
    ...
    props.setHeader({
      headerContentElement: (
        <DefaultHeaderContent
          actions={
            <>
              <Button onClick={() => setNewAlbumDialogOpen(true)} label="Create album" icon={<IconCreate />} />
            </>
          }
        />
      ),
  ...
  return (
    <Content>
      <TagGroupedAlbums albums={sortedAlbums} onAlbumClick={(album) => context.history.push("/album/" + album.id)} />

      <ModalCreateAlbum isOpen={newAlbumDialogOpen} onRequestClose={() => setNewAlbumDialogOpen(false)} />
    </Content>
  );
};
