...
const TagGroupedAlbums: FC<Props> = (props: Props) => {
  ...
  return (
    <>
      ...
      {tags.map((tag) => {
        const albumsWithTag = props.albums.filter((album) => album.tags.some((t) => t === tag));

        return <AlbumTag key={tag} tag={tag} albums={albumsWithTag} onAlbumClick={props.onAlbumClick} />;
      })}

      {/* Also render all albums that do not have any tags */}
      {albumsWithoutTag.length > 0 && (
        <AlbumTag tag="no-tag" albums={albumsWithoutTag} onAlbumClick={props.onAlbumClick} />
      )}
    </>
  );
};
