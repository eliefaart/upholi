...
const ExifData: FC<Props> = (props) => {
	...
	let exposureText = "";
	if (props.exif.aperture)
		exposureText += props.exif.aperture + " ";
	if (props.exif.exposureTime)
		exposureText += props.exif.exposureTime + " ";
	if (props.exif.iso)
		exposureText += "ISO-" + props.exif.iso;
	...
	return !!props.exif && <div className="exif">
		{!!props.exif.manufactorer && <div className="property">
			<span className="name">Camera</span>
			<span className="value">{props.exif.manufactorer} {props.exif.model}</span>
		</div>}
