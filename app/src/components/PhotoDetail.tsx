...
class PhotoDetail extends React.Component<PhotoDetailProps, PhotoDetailState> {
  ...
  zoomPhoto(imgElement: HTMLElement, zoomDelta: number, zoomStyle: number): void {
    if (!zoomDelta || zoomDelta === 0) return;
    ...
    const currentScaleFactor = matches && matches.length >= 2 ? parseFloat(matches[1]) : 1;
    ...
    const newScaleFactor = zoomingIn ? currentScaleFactor + zoomStep : currentScaleFactor - zoomStep;
    ...
    return (
      <div className="photo-detail">
        ...
        {this.props.isVideo ? (
          ...
          <img src={this.props.src} className="photoDetailImg" draggable={false} style={{ top: "0px", left: "0px" }} />
        )}
      </div>
    );
  }
