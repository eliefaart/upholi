    ...
    <>
      ...
      <nav>
        {menuItems.map((menuItem) => {
          const isActive =
            location.pathname === menuItem.path || menuItem.childPaths.some((p) => location.pathname.startsWith(p));
          return (
            <span
              key={menuItem.path}
      ...
      {!!props.contextMenu && contextMenuOpen && (
        <div className="context-menu" onClick={() => setContextMenuOpen(false)}>
          <div className="items">{props.contextMenu}</div>
        </div>
      )}

      {/* Context menu toggle button */}
      {!!props.contextMenu && (
        <button className="context-menu-toggle" onClick={() => setContextMenuOpen(!contextMenuOpen)}>
          <IconContextMenu />
        </button>
      )}
    </>
