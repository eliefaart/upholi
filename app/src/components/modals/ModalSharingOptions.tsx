...
const ModalSharingOptions: FC<Props> = (props) => {
  ...
  return (
    ...
    >
      <Switch checked={shared} label="Share via URL" onChange={setShared} />

      {shared && <Switch checked={passwordProtected} label="Require password" onChange={setPasswordProtected} />}

      {shared && passwordProtected && (
        <label>
          Password
          <input
            type="text"
