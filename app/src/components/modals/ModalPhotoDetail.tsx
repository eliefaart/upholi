...
const ModalPhotoDetail: FC<Props> = (props) => {
  ...
  React.useEffect(() => {
    upholiService.getPhoto(props.photoId).then(setPhoto).catch(console.error);

    upholiService.getPhotoPreviewImageSrc(props.photoId).then(setPhotoSrc).catch(console.error);
  }, [props.photoId]);

  const headerActions = (
    <>
      {photo?.exif && <Button onClick={() => setShowExif(!showExif)} label="Info" icon={<IconInfo />} />}
      <Button onClick={() => downloadPhoto(props.photoId)} label="Download" icon={<IconDownload />} />
    </>
  );
